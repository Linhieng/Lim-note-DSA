...
搜索二叉树 BST(Binary Search Tree): 对于任一子树, 左子节点的值 < 父节点的值, 右子节点的值 > 父节点的值。
...
- 条件2: 在条件1没有违规过的前提下, 一旦遇到子节点未被填满, 后续层序遍历只允许出现叶节点, 否则返回 false
...
- 情况2: o1 或 o2 不互为最低公共祖先。 这种情况需要从两个节点往上汇聚的时候才能找到公共祖先。
...
Node lowestAncestor(Node head, Node o1, Node o2) {
    ...
    if (left != null && right != null) { // 对于情况1, 这个 if 不可能命中
        ...
        只有当 左侧找到 o1, 右侧找到 o2 时, 才会出现 left 和 right 同时不为空的情况2
...
序列化方式有多种, 比如以先序方式遍历二叉树, 将识别到的值(包括空)都存储起来。
